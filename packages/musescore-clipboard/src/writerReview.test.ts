...
function track(partOffset = 0, staffOffset = 0, voiceIndex = 0): MuseScoreClipboardTrack {
  return { partOffset, staffOffset, voiceIndex, content: notes() };
}

function selection(overrides: Partial<MuseScoreClipboardWriteInput> = {}): MuseScoreClipboardWriteInput {
  return {
    events: notes(),
    ...overrides,
  };
...
function mf(): MuseScoreClipboardDynamic {
  return {
    measureOffset: 0,
    offset: [0, 1],
    dynamic: {
...
function exported(selection: MuseScoreClipboardWriteInput): Document {
...
describe("MuseScore writer annotation review", () => {
  ...
  it("preserves separate occurrences while recognizing equivalent captured offsets", () => {
    const first = mf();
    const equivalent: MuseScoreClipboardDynamic = { ...mf(), offset: [0, 4] };
    const later: MuseScoreClipboardDynamic = { ...mf(), offset: [1, 4] };
    const document = exported(
      selection({
        tracks: [

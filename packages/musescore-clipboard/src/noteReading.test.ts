...
describe("MuseScore explicit Note accidental source contract", () => {
  it("reads the supplied B3 natural without replacing concert TPC or assuming intentional user role", () => {
    const notes = Array.from(parseSafeXml(NATURAL_CAPTURE).getElementsByTagName("Note")).map((element) =>
...
describe("MuseScore explicit accidental XML writing", () => {
  ...
  it.each([
    ...
    // Native vendor dictionaries can contain fields the generated core schema does not type.
  ] as unknown as AccidentalDisplay[])(

...
interface ExportTrack extends MuseScoreClipboardTrack {
  staff: number;
  voice: number;
}
...
function annotationOffset(captured: MuseScoreClipboardDynamic | MuseScoreClipboardChordSymbol, path: string): Fraction {
...
function dynamicXml(captured: MuseScoreClipboardDynamic, path: string): string {
...
function annotationsByTrack(
  ...
  const appendDynamic = (captured: MuseScoreClipboardDynamic, sourcePartOffset: number, path: string): void => {
...
function exportTracks(selection: MuseScoreClipboardWriteInput): ExportTrack[] {
  const sourceTracks =

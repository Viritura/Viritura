...
export function captureSelectedDynamics(
  ...
  return locations.flatMap((location) => {
    ...
    if (location.measureIndex === origin.measureIndex) {
      cloned.position = { fraction: exactCaptureDifference(beats, origin.beat) };
    }
    ...
    if (cloned.type === "gradual") {
      ...
      endOffset = exactCaptureDifference(
        beatsBetweenMeasures(score, origin.measureIndex, endMeasure) + endBeat,
        origin.beat,
      );
      if (endMeasure === origin.measureIndex) {
        cloned.end.position = {
          fraction: exactCaptureDifference(endBeat, origin.beat),
        };
      }
    }
    const onset = beatsBetweenMeasures(score, origin.measureIndex, location.measureIndex) + beats;
    return [
      {
        ...
        endMeasureOffset,
        offset: exactCaptureDifference(onset, origin.beat),
        ...(endOffset ? { endOffset } : {}),
        dynamic: cloned,
      },
    ];
...
export function collectDynamics(
  ...
  sourceRanges?: readonly SelectionTrackRhythmicRange[],
): CapturedDynamic[] {
  ...
  for (
    ...
    for (const dynamic of dynamics.filter((item) => !sourceStaves || sourceStaves.has(item.staff ?? 1))) {
      ...
      if (!annotationInTrackRanges(sourceRanges, partIndex, dynamic.staff ?? 1, measureIndex, beats)) continue;
      ...
      if (isFirst && firstMeasureStartBeat > 0) {
        cloned.position = { fraction: exactCaptureDifference(beats, firstMeasureStartBeat, 16) };
      }
      let endMeasureOffset: number | undefined;
      let endOffset: [number, number] | undefined;
      if (cloned.type === "gradual") {
        ...
        endOffset = exactCaptureDifference(
          beatsBetweenMeasures(score, startMeasure, endMeasureIndex) + endBeats,
          firstMeasureStartBeat,
        );
        if (endMeasureIndex === startMeasure && firstMeasureStartBeat > 0) {
          cloned.end.position = { fraction: exactCaptureDifference(endBeats, firstMeasureStartBeat, 16) };
        }
      }
      result.push({
        measureOffset: measureIndex - startMeasure,
        endMeasureOffset,
        offset: exactCaptureDifference(beatsBeforeMeasure + beats, firstMeasureStartBeat),
        ...(endOffset ? { endOffset } : {}),
        dynamic: cloned,
      });
    }

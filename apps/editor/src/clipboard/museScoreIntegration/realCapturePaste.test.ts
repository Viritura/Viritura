...
function fixture(name: string): string {
  return readFileSync(
    new FileURL(`../../../../../packages/musescore-clipboard/fixtures/realCaptures/${name}.xml`, import.meta.url),
    "utf8",
  );
}

const doublebassXml = fixture("doublebass");
const hairpinXml = fixture("hairpin");
const naturalXml = fixture("natural");
const pianoXml = fixture("pianoLowerDynamics");
const tiesXml = fixture("ties");
    ...
    parts: layout.map((staves, part) => ({
      ...
      measures: Array.from({ length: 8 }, (_, measure) => ({
        sequences: Array.from({ length: staves }, (_, staff) => ({
          staff: staff + 1,
          content: Array.from({ length: measureBeats(time) * 4 }, (_, event): NoteEvent => ({
            type: "event",

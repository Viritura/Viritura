...
/** All lyric lines in their configured order, followed by any unconfigured lines. */
...
export function getInitialLyricLineId(score: Score): string {
  return getLyricLineIds(score)[0] ?? "1";
}

export function getLyricLineDisplay(score: Score, lineId: string): string {
  const label = score.global.lyrics?.lineMetadata?.[lineId]?.label;
  if (label) return label;
  const index = getLyricLineIds(score).indexOf(lineId);
  if (index >= 0) return `verse ${index + 1}`;
  return `verse ${lineId.replace(/^line-/, "")}`;
}
...
/** BCP 47 syntax validation without canonicalizing or otherwise rewriting the tag. */
export function isValidLanguageTag(value: string): boolean {
  ...
  const singletonIndexes = subtags.flatMap((part, index) =>
    part.length === 1 && part !== "x" && index > 0 ? [index] : [],
  );
  return new Set(singletonIndexes.map((index) => subtags[index])).size === singletonIndexes.length;
}

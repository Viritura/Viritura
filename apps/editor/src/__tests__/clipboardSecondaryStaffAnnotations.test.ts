...
describe("secondary physical staff annotation capture", () => {
  it("recaptures a lower-staff hairpin ending exactly at a rebarred 6/8 selection end", () => {
    const paste = importedPiano(true, 2);
    paste.dynamics!.push({
      partOffset: 0,
      staffOffset: 1,
      measureOffset: 0,
  ...
  ])("routes the decoded piano into $layout and recaptures once across voices", ({ layout, lowerPart, lowerStaff }) => {
  ...
  it.each(["single", "range", "placed"] as const)("copies only lower-staff annotations for a %s selection", (kind) => {
    const source = applyPaste(score([1, 2]), importedPiano(true), 1, 1, 0, 0);
  ...
  it.each(["single", "range", "placed"] as const)(
    ...
    (kind) => {
      const source = applyPaste(score([2]), importedPiano(true), 0, 0, 0, 0);

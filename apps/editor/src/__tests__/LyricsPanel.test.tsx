...
import { LyricsPanel } from "../components/modes/setup/LyricsPanel";
...
describe("Lyrics settings panel", () => {
  it("exposes accessible metadata fields, mutates the score, and serializes explicit order", async () => {
    const user = userEvent.setup();
    renderPanel();

    const label = await screen.findByRole("textbox", { name: "Label for line-a" });
    expect(screen.getByRole("textbox", { name: "Language for line-a" })).toBeTruthy();

    await user.clear(label);
    await user.type(label, "Lead");
    await user.tab();
    const language = screen.getByRole("textbox", { name: "Language for line-b" });
    await user.type(language, "qaa-Qaaa-QM-x-studio");
    await user.tab();
    await user.click(screen.getByRole("button", { name: "Move line-b up" }));
  ...
  it("rejects malformed language tags without rewriting valid private-use tags", async () => {
    const user = userEvent.setup();
    renderPanel();
    const language = await screen.findByRole("textbox", { name: "Language for line-b" });

-- Opus "simple violin" mapper — Viritura's canonical example articulation map.
--
-- The patch is selected by MIDI channel (one channel per articulation), dynamics
-- ride on CC11, and con sordini rides on CC15. Every callback receives the
-- timeline `time` (seconds) of its event; MIDI is scheduled relative to that time
-- and any event before the origin is clamped to 0.
--

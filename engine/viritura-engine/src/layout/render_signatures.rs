...
pub(crate) fn key_signature_layout(
    ...
    let (sharp_positions, flat_positions): (&[f64; 7], &[f64; 7]) = match clef_sign {
        ClefSign::F => (
            &[2.0, 5.0, 1.5, 4.5, 7.5, 3.5, 6.5],
            &[6.0, 3.0, 6.5, 3.5, 7.0, 4.0, 7.5],
        ),
        ClefSign::C => (
            &[1.0, 4.0, 0.5, 3.5, 6.5, 2.5, 5.5],
            &[5.0, 2.0, 5.5, 2.5, 6.0, 3.0, 6.5],
        ),
        _ => (
            &[0.0, 3.0, -0.5, 2.5, 5.5, 1.5, 4.5],
            &[4.0, 1.0, 4.5, 1.5, 5.0, 2.0, 5.5],
        ),
    };
